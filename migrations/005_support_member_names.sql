-- Resolve support-member names from Jira user records, while preserving view columns.
...
SELECT w.tempo_id AS tempo_worklog_id, w.issue_id AS jira_issue_id,
       ...
       COALESCE(NULLIF(u.payload->>'displayName', ''),
                NULLIF(u.payload->>'display_name', ''),
                NULLIF(u.payload->>'name', ''),
                s.display_name, w.author_id) AS support_member,
...
LEFT JOIN users u ON u.tempo_id = w.author_id
